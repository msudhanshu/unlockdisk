...
public class Cone extends CommonGeometry{

    public Cone(float radius,float height,int SPHERELONG_SEGMENT,int SPHERELAT_SEGMENT) {
    	initializeBuffer (makeConeGeometry(radius, height, SPHERELONG_SEGMENT, SPHERELAT_SEGMENT));
    }
    ...
    public static GeometryParameterInArray makeConeGeometry (float radius, float height, int SPHERELONG_SEGMENT, int SPHERELAT_SEGMENT) {
    	...
    	int NR_OF_SEGMENTS = Math.max(SPHERELAT_SEGMENT, SPHERELONG_SEGMENT);
    	  for (int latNumber = 0; latNumber <= NR_OF_SEGMENTS; latNumber++) {
    	      ...
    	      for (int longNumber = 0; longNumber <= NR_OF_SEGMENTS; longNumber++) {
    	    	  ...
    	    	  float currentRadius = ch*radius/height;
    	       	  float x = cosPhi;
    	    	  float y = ch;
    	    	  float z = sinPhi ;
    	    	  ...
    	    	  mGeometryParameter.geometryData.add(currentRadius * z);
    	      }
